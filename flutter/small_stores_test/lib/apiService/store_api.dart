...
class StoreApi {
  ...
  Future<StoreModel> addStore({
    ...
    if (store_photo != null) {
      request.files.add(await http.MultipartFile.fromPath('store_photo', store_photo.path));
    } else if (storePhotoBytes != null) {
      request.files.add(http.MultipartFile.fromBytes('store_photo', storePhotoBytes, filename: "upload.png"));
    }
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      return StoreModel.fromJson(jsonResponse['data']);
    } else {
      String errorMessage = "فشل إضافة المتجر";
      try {
        final jsonResponse = jsonDecode(respStr);
        if (jsonResponse is Map) {
          ...
          if (jsonResponse.containsKey('errors')) {
            ...
            if (errors.isNotEmpty) {
              errorMessage = errors.values.first[0]; // <-- هنا
            }
          }
        }
      } catch (_) {
        errorMessage = "فشل العملية: ${response.reasonPhrase}";
      }
  ...
  Future<StoreModel> updateStore({
    ...
    if (response.statusCode == 200) {
      ...
      final Map<String, dynamic> jsonResponse = jsonDecode(respStr);
      final firstError = jsonResponse['errors'].values.first[0];
      throw Exception(firstError);
    } else {
      throw Exception("فشل تعديل المتجر: ${response.reasonPhrase}");
    }
  }
